    ...
    try {
        if (escapeKey == event.keyCode) {
            ...
            if (CurrentCutItem != null) {
                ...
                contextMenu.children["PasteItem"].innerHTML = '<img src="content_manager/images/paste.png" border="0" alt="" height="16" width="16" style="margin-right:12px;vertical-align:middle;"/>Paste';
...
function confirmDelete(whichObject) {
    ...
    if (whichObject == 'directory') {
        if (confirm("Profile: " + document.frmEditMenu.f_name.value + " " + document.frmEditMenu.l_name.value + "\n\nAre you sure you want to delete this profile?")) {
            return true;
        }
        else {
            return false;
    ...
    if (whichObject == 'acronym') {
        if (confirm("Acronym: " + document.frmEditMenu.english.value + " - " + document.frmEditMenu.desc_e.value + "\n\nAre you sure you want to delete this acronym?")) {
            return true;
        }
        else {
            return false;
 ...
 switch (v)
 {
	...
	case "br":
 	...
 	case "space":
		...
		break;
	case "img":
		var rValue=window.showModalDialog("content_manager/content_Manager_Pages/sam_image_selector.asp","", "dialogHeight: 650px; dialogWidth: 840px; edge: Raised; center: Yes; help: No; resizable: Yes; status: No;");
	...
	case "a":
		var obj = new Object();
    	obj.tag = v;
    	var rValue = window.showModalDialog("content_manager/content_Manager_Pages/sam_editor_selector.asp", obj, "dialogHeight: 380px; dialogWidth: 530px; edge: Raised; center: Yes; help: No; resizable: No; status: No;");
		...
		break;
	case "pgid":
	    var rValue = window.showModalDialog("content_manager/content_Manager_Pages/sam_page_selector.asp", "", "dialogHeight: 600px; dialogWidth: 840px; edge: Raised; center: Yes; help: No; resizable: Yes; status: No;");
		if(rValue=='cancel' || rValue==undefined || rValue=='')
		{
			document.frmEditMenu.stream.focus();
			return;
...
function removeGroup(groupid, empid) {
    var rValue = window.showModalDialog("content_manager/content_manager_pages/sam_directory_group_selector.asp?op=delete&groupid=" + groupid + "&empid=" + empid, "", "dialogHeight: 180px; dialogWidth: 430px; edge: Raised; center: Yes; help: No; resizable: No; status: No;");
    if (rValue == 'cancel') {
        return;
    }
    else if (rValue == 'yes') {
...
function loadAssistants() {
    ...
    var rValue = window.showModalDialog("content_manager/content_manager_pages/sam_directory_assistant_selector.asp", obj, "dialogHeight: 450px; dialogWidth: 420px; edge: Raised; center: Yes; help: No; resizable: No; status: No;");
    if (rValue != 'cancel') {
...
function loadGroups(empid) {
    var rValue = window.showModalDialog("content_manager/content_manager_pages/sam_directory_group_selector.asp?op=add&empid=" + empid, "", "dialogHeight: 580px; dialogWidth: 420px; edge: Raised; center: Yes; help: No; resizable: No; status: No;");
    if (rValue == 'cancel') {
        return;
    }
    else if (rValue == '') {
...
function editorPrev()
{
	...
	var rValue=window.showModalDialog("contentManagerPages/mcm_content_editor_preview.asp",obj, "dialogHeight: 435px; dialogWidth: 700px; edge: Raised; center: Yes; help: No; resizable: Yes; status: No;");
}
...
function CutTreeViewItem()
{
	...
	{
		if(colAnc[i].ItemId==contextMenu.srcId)
	...
	contextMenu.children["PasteItem"].innerHTML = '<img src="content_manager/images/paste.png" border="0" alt="" height="16" width="16" style="margin-right:12px;vertical-align:middle;"/>Paste ';
	contextMenu.children["PasteItem"].id=CurrentCutItem.ItemId;
...
function CutContainer() {
    ...
    for (i = 0; i < colAnc.length; i++) {
        if (colAnc[i].ContainerId == contextMenu.srcId) {
            CurrentCutItem = colAnc[i];
    ...
    if (CurrentCutItem == null) {
        for (i = 0; i < colAnc.length; i++) {
            if (colAnc[i].ImageId == contextMenu.srcId) {
                CurrentCutItem = colAnc[i];
...
function showContextMenu(vLevel)
{
	...
	if(CurrentCutItem==null || (CurrentCutItem!=null && src.ItemId==CurrentCutItem.ItemId) || src.nodeType=='leaf')
	{
	...
	for(i=0;i<AncTags.length;i++)
	{
		if(AncTags[i].vid=="xc")
		{
		    if (event.srcElement.init == "1") {
		        ...
		        AncTags[i].innerHTML = "<img src='content_manager/images/minus.gif' border='0' style='margin-right:4px;'/>";
...
function validateAcronym() {
    ...
    if (document.frmEditMenu.english.value == '') {
        submitFailed = true;
        engAcronym = "Acronym (eng) field cannot be blank.\n";
    }
    if (document.frmEditMenu.french.value == '') {
        submitFailed = true;
        freAcronym = "Acronym (fre) field cannot be blank.\n";
    }
    if (document.frmEditMenu.desc_e.value == '') {
        submitFailed = true;
        engTitle = "Description (eng) field cannot be blank.\n";
    }
    if (document.frmEditMenu.desc_f.value == '') {
        submitFailed = true;
...
function scrollToNode()
{
	...
	{
		var src = "scrollTo" + document.all.oHighlightNode.value;
...
function CollapseSection(vulid, isSelector) {
	...
	for(i=0;i<ulidTags.length;i++) {
		...
		if(ulidTags[i].ulid==vulid)
		{
			if(ulidTags[i].style.display=="none")
			{
			    ulidTags[i].style.display = "";
				srcElm.innerHTML = "<img src='"+imgSource+"content_manager/images/minus.gif' border='0' style='margin-right:4px;'/>";
			}
			else
			{
			    ulidTags[i].style.display="none";
				srcElm.innerHTML = "<img src='"+imgSource+"content_manager/images/plus.gif' border='0' style='margin-right:4px;'/>";
			}
		}
	}
...
function SetCollapseExpandSections()
{
	...
	for(i=0;i<ulidTags.length;i++)
	{
		if(ulidTags[i].top)
		{
			var colId ="ExpCol"+ulidTags[i].ulid;
	        
			if(readCookie(colId)==null)
			    setCookie(colId, "Expanded", 365);

			var expColText = readCookie(colId);
			var ancElement = document.getElementById(colId);
			if (expColText == "Collapsed")
			{
			    ancElement.innerHTML = "<img src='content_manager/images/plus.gif' border='0' style='margin-right:4px;'/>";
				ulidTags[i].style.display="none";
			}
			else
			{
			    ancElement.innerHTML = "<img src='content_manager/images/minus.gif'  border='0' style='margin-right:4px;'/>";
				ulidTags[i].style.display="";
			}
		}
	}
...
function launchEditorWindow(id,inc,title)
{
    window.open('editor.asp?id=' + id + '&inc=' + inc + '&title=' + title, '', 'fullscreen=no,directories=yes,location=no,status=yes,menubar=yes,resizable=yes,scrollbars=yes');
}
function endSession()
{
